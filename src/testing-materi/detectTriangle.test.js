...
describe('detectTriangle function', ()=> {
  ...
  it('shoud throw error when input is not a positive number', () => {
    expect(() => detectTriangle(-1, 2, 3)).toThrowError('Input must be a positive number');
    expect(() => detectTriangle(1, -2, 3)).toThrowError('Input must be a positive number');
    expect(() => detectTriangle(1, 2, -3)).toThrowError('Input must be a positive number');
    // Zero is not positif and negative number
    expect(() => detectTriangle(0, 2, 3)).toThrowError('Input must be a positive number');
  });

  it('shoud throw error when input is triangle inequality violation', () => {
  ...
  it('shoud return "Segitiga Sembarang" when all input are different', () => {
